...
POTSDAM_DATA_PATH = '../../dataset/Potsdam_split_data2/train/'
...
def main():
    ...
    parser.add_argument('--model_type', required=True, default="DeepLabv3",
                        help="declare the model type to use, currently only support input being DeepLabv3")
    ...
    model = deeplabv3plus_model(num_channel=num_channel, num_classes=num_classes)
    ...
    for epoch in range(num_epoch):
        ...
        for phase in ['train', 'test']:
            ...
            for i, d in enumerate(dataloaders[phase]):
                ...
                #dice_loss = criterion(output, semantic_seg_mask.long())
                dice_loss = diceloss(output, semantic_seg_mask.float(), softmax=True)
                #dice_loss = 0.4 * criterion(output, semantic_seg_mask.long()) + 0.6 * diceloss(output,semantic_seg_mask.float(),softmax=True)

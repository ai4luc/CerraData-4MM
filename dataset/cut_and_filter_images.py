...
# Loop for clipping
for raster in glob.iglob('./Potsdam/top_potsdam_2_10_label_noBoundary.tif'):
    ...
    if os.path.isdir(name_file) == False:
        os.mkdir('./Potsdam/mask/' + name_file + '_clipped')
    ...
    for i in range(xdiv):
        for j in range(ydiv):
            ...
            # use gdal warp
            gdal.Warp('./Potsdam/mask/' + name_file + '_clipped' + '/' + name_file+'_' + str(i) + str(j) + ".tif",
                      file, outputBounds=(xmin, ymin, xmax, ymax), dstNodata=None)

            # or gdal translate to subset the input raster
            gdal.Translate('./Potsdam/mask/' + name_file + '_clipped' + '/' + name_file+'_' + str(i) + str(j) + ".tif",
